...
startup {
	...
	vars.journalEntries.Add(
		new System.Text.RegularExpressions.Regex(@"\{ ""timestamp"":""(?<timestamp>.*)"", ""event"":""Undocked"", ""StationName"":""Hoshide Dock"", ""StationType"":"".*"", ""MarketID"":\d+(, ""Taxi"":(true|false), ""Multicrew"":(true|false))? \}"));
	for (int i=0; i<6; i++) {
		vars.journalEntries.Add(
			new System.Text.RegularExpressions.Regex(@"\{ ""timestamp"":""(?<timestamp>.*)"", (""event"":""SupercruiseExit""(, ""Taxi"":(true|false), ""Multicrew"":(true|false))?, ""StarSystem"":""(Beatis|Nauani|Vodyanes)"", ""SystemAddress"":\d+, ""Body"":""(Neff Hub|Nauani B 1|Lewis Gateway)"", ""BodyID"":\d+, .*\ |""event"":""SupercruiseDestinationDrop"", ""Type"":""(White Grove Homestead|Intermutual Interchange Core|Conjunct Transmittal Satellite)"", ""Threat"":\d+, ""MarketID"":(3240886785|3247183873|3246504193).*)\}"));
		vars.journalEntries.Add(
			new System.Text.RegularExpressions.Regex(@"\{ ""timestamp"":""(?<timestamp>.*)"", ""event"":""StartJump"", ""JumpType"":""(Hyperspace|Supercruise)"", .*\}"));
	}
	vars.journalEntries.Add(
		new System.Text.RegularExpressions.Regex(@"\{ ""timestamp"":""(?<timestamp>.*)"", ""event"":""FSDJump""(, ""Taxi"":(true|false), ""Multicrew"":(true|false))?, ""StarSystem"":""Adivarakhe"", ""SystemAddress"":22974316694288, .*\}"));
	vars.journalEntries.Add(
		new System.Text.RegularExpressions.Regex(@"\{ ""timestamp"":""(?<timestamp>.*)"", ""event"":""SupercruiseExit""(, ""Taxi"":(true|false), ""Multicrew"":(true|false))?, ""StarSystem"":""Adivarakhe"", ""SystemAddress"":22974316694288, ""Body"":""Hoshide Dock"", ""BodyID"":\d+, ""BodyType"":""Station"" \}"));
	vars.journalEntries.Add(
		new System.Text.RegularExpressions.Regex(@"\{ ""timestamp"":""(?<timestamp>.*)"", ""event"":""Docked"", ""StationName"":""Hoshide Dock"", ""StationType"":"".*""(, ""Taxi"":(true|false), ""Multicrew"":(true|false))?, ""StarSystem"":""Adivarakhe"", .*\}"));
